...
- ✅ **Dark / light theme** — View > Theme > Light / Dark / System; applied immediately to
  every open window and to dialogs opened afterwards, no restart. "System" follows the OS
  appearance live. The selection persists in `~/.pgadmin3-javafx-reborn/preferences.json`
- ✅ Complete second icon set for dark mode (89 icons under `icons/dark/`, generated by
  `tools/DarkIconGenerator.java` and audited for legibility); SQL highlighting, the
  graphical EXPLAIN diagram and its glyphs all restyle with the theme
- ✅ Editor body text (everything the highlighter does not tokenize) and checked menu marks
  are themed explicitly rather than inherited from JavaFX/Modena defaults, which were
  unreadable in dark; popups follow a live theme switch
- ✅ Both palettes meet WCAG AA (4.5:1 text, 3:1 graphics), asserted by `ThemeContrastTest`;
  the editor's real rendered text fills are asserted by `EditorTextFillTest`
- ℹ️ pgAdmin III 1.22 had no dark mode — this is a deliberate addition; the light theme
...
| Model | `ServerInfo`, `ServerRegistry`, `AppPreferences` | JSON persistence of registrations and settings |
...
| UI | `MainWindow`, `DetailPane`, `ThemeManager` | menus, toolbar, context menus, 4 detail tabs + SQL pane, app-wide theming |
